...
def getPhi(ins_prev, mear_cur, mear_prev):  # sk cur skp pre
    ...
    gp = fnc.NormalGravity(phi, h)

    Rm = fnc.GetRM(a, e2, phi)
    Rn = fnc.GetRN(a, e2, phi)

    w_ie_n = fnc.GetW_ie(we, r)
    w_en_n = fnc.GetW_en(r, v, Rn, Rm)
    ...
    F[3:6, 0:3] = Fvr
    F[3:6, 3:6] = Fvv
    F[3:6, 6:9] = fnc.cp_form(Cbn.dot(fb))
    F[3:6, 12:15] = Cbn
    F[3:6, 18:21] = Cbn.dot(np.diag([fb[0, 0], fb[1, 0], fb[2, 0]]))
    F[6:9, 0:3] = Ffr
    F[6:9, 3:6] = Ffv
    F[6:9, 6:9] = -fnc.cp_form(w_in_n)
    F[6:9, 9:12] = -Cbn
...
def INS_MECH(meas_pre, meas_cur, nav, par, imu_paramters):
    ...
    sg = np.diag([imu_paramters[0, 2], imu_paramters[1, 2], imu_paramters[2, 2]])
    sa = np.diag([imu_paramters[0, 3], imu_paramters[1, 3], imu_paramters[2, 3]])
    I_3 = np.diag([1] * 3)
    meas_cur[1:4, 0] = np.linalg.inv(I_3 + sg).dot(meas_cur[1:4, 0] - bg * dt)
    meas_cur[4:7, 0] = np.linalg.inv(I_3 + sa).dot(meas_cur[4:7, 0] - ba * dt)
...
def predict(xk_1, pk_1, ins_state_cur, ins_state_pre, mear_cur, mear_prev):  # ins_state惯导当前状态 ins_state_pre前一时刻
    ...
    Qk_1 = (phi.dot(G_k_1).dot(Q0).dot(G_k_1.T).dot(phi.T) + G_k.dot(Q0).dot(G_k.T)) * dt / 2
...
def state_back(xk, ins_state_cur):  # 位置、速度、姿态反馈
    ...
    Rm = fnc.GetRM(a, e2, lat)
    Rn = fnc.GetRN(a, e2, lat)
    correct = np.linalg.inv(DR(Rm, Rn, h, lat)).dot(xk[0:3, :])
    ins_state_cur.r[0:3, 0] = ins_state_cur.r[0:3, 0] - correct
    ins_state_cur.q_ne = fnc.pos2quat(ins_state_cur.r[0, 0], ins_state_cur.r[1, 0])
    ins_state_cur.q_ne = fnc.norm_quat(ins_state_cur.q_ne)
    # 姿态反馈
    Ctp = np.diag([1, 1, 1]) - fnc.cp_form(xk[6:9, :])
    Cpt = np.linalg.inv(Ctp)
    Cbn = ins_state_cur.C_bn.copy()
    ins_state_cur.C_bn = Cpt.dot(Cbn)
    ins_state_cur.q_bn = fnc.dcm2quat(ins_state_cur.C_bn)
    ins_state_cur.q_bn = fnc.norm_quat(ins_state_cur.q_bn)
